...
export default function ProductCard({ product }) {
  ...
  return (
    <Link to={`/shop/${product.id}`} className="group relative flex flex-col overflow-hidden rounded-lg border border-gray-200 bg-white transition-all duration-300 hover:shadow-2xl focus:outline-none focus:ring-2 focus:ring-blue-500 focus:ring-offset-2">
      ...
      <div className="flex flex-1 flex-col space-y-2 p-4">
       ...
       <div className="p-4 pt-0 z-10">
         <div className="flex items-center justify-center gap-2">
           <div className={`flex items-center justify-center rounded-full bg-gray-100 shadow-sm flex-shrink-0 ${isOutOfStock ? 'opacity-50' : ''}`}>
             <button onClick={handleDecrease} className="p-2.5 text-gray-700 rounded-l-full hover:bg-gray-200 disabled:cursor-not-allowed" disabled={quantity === 0 || isOutOfStock}>
               <MinusIcon className="h-5 w-5" />
             </button>
             <span className="px-2 text-center font-bold text-gray-900 text-base w-8">{quantity}</span>
             <button onClick={handleIncrease} className="p-2.5 text-gray-700 rounded-r-full hover:bg-gray-200 disabled:cursor-not-allowed" disabled={isOutOfStock}>
